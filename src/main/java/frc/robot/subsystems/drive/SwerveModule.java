...
public class SwerveModule {
  ...
  public void setDesiredState(SwerveModuleState desiredState, boolean closedLoop) {
    // Make sure the desired state's angle is in the range [-180, 180], so it can be
    // appropriately
    ...
    desiredState = constrainState(desiredState); // constrain one more time after optimization just to be safe, because
                                                 // I'm unsure if optimization can ever pull the angle out of [-180,
                                                 // 180]
  ...
  public void setDesiredStateNoOptimize(SwerveModuleState desiredState, boolean closedLoop) {
    if (closedLoop) {
      // Conversion factor is already set below to convert rpm of motor to m/s of
      // wheel.
